...
* This plugin is based on sms-twilio plugin (UCRM part) and tlapi/gosms library
...
* [GoSMS](https://www.gosms.cz/) account is required to access its API.

## Configuration

* Install the plugin into UCRM and enable it. I.e. download the plugin [zip file](https://github.com/vojapet/UCRM-plugins/raw/master/plugins/sms-gosms/sms-gosms.zip) and upload it to UCRM in System > Plugins.
* Keep execution period at "don't execute automatically" - the plugin will react to webhook events.
* Set up with data which you obtain from [GoSMS API description](https://app.gosms.cz/selfservice/api/):
   * Client ID
   * Client Secrete
   * SMS channel
...
* Click Test Endpoint
* Go to System / Plugins / SMS notifications via GoSMS
